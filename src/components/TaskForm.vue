<template>
  <div class="w-full max-w-3xl mx-auto flex justify-between items-center mb-6 px-4 sm:px-0">
    ...
    <div class="flex gap-4">
      <button @click="$emit('change-tab', 'all')"
        :class="['px-4 py-2 font-medium', activeTab === 'all' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500']">
        My Tasks
      </button>
      <button @click="$emit('change-tab', 'complete')"
        :class="['px-4 py-2 font-medium', activeTab === 'complete' ? 'border-b-2 border-green-500 text-green-600' : 'text-gray-500']">
        Completed
      </button>
    </div>

    <!--add task delete all-->
    <button v-if="activeTab === 'all'" @click="showModal = true"
    ...
    <button v-else @click="confirmDeleteAll"
      class="flex items-center gap-2 px-3 py-1.5 bg-red-500 text-white text-sm font-medium rounded hover:bg-red-600 transition">
...
const props = defineProps<{ activeTab: 'all' | 'complete' }>()
const emit = defineEmits<{
  (e: 'change-tab', tab: 'all' | 'complete'): void
}>()
...
const confirmDeleteAll = () => {
  if (confirm("คุณแน่ใจหรือไม่ว่าต้องการลบ Completed tasks ทั้งหมด?")) {
